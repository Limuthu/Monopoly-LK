...
void print_square(GameState *game, int index) {
  ...
  case SQUARE_PROPERTY:
    ...
    printf("Price: LKR %.0f | Mortgage: LKR %.0f\n", sq.data.property.price,
           sq.data.property.mortgage_value);
    ...
    printf("Owner ID: %d | Houses: %d | Has Hotel: %d\n",
           sq.data.property.owner_id, sq.data.property.num_houses,
           sq.data.property.has_hotel);
    break;

  case SQUARE_RAILWAY:
    printf("Type: RAILWAY\n");
    printf("Price: LKR %.0f | Owner ID: %d\n", sq.data.railway.price,
           sq.data.railway.owner_id);
    break;

  case SQUARE_UTILITY:
    printf("Type: UTILITY\n");
    printf("Price: LKR %.0f | Owner ID: %d\n", sq.data.utility.price,
           sq.data.utility.owner_id);
    break;
